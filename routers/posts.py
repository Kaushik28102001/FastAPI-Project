...
async def get_post_dates(db: DBSession):
    result = await db.execute(
...
async def upload_post_picture(
    ...
    try:
        new_filename = await run_in_threadpool(process_post_image, content)
...
async def get_comments(
    ...
    result = await db.execute(
        ...
        .where(models.PostComment.post_id == post_id)
...
async def get_like_status(
    ...
    return {"likes": like_count, "liked": liked}
...
async def toggle_like(
    ...
    return {"action": action, "likes": like_count}
...
async def toggle_repost(
    ...
    count_result = await db.execute(
        select(func.count(models.Repost.id)).where(models.Repost.post_id == post_id)
    )
    repost_count = count_result.scalar() or 0

    return {"action": action, "reposts": repost_count}