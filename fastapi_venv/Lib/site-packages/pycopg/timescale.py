...
from pycopg.exceptions import ExtensionNotAvailableError, TimescaleError
...
def _build_time_bucket_sql(
    ...
    """Build a ``time_bucket`` bucketed-aggregation SELECT (D-01, D-05, TSDB-F02).

    Identifiers (``table``, ``time_column``, ``schema``) are validated and
    interpolated; ``bucket_width``, ``origin``, and ``offset`` are bound as
    ``%s`` parameters.  The bucket expression always carries the fixed
    ``AS bucket`` alias so the output column is deterministically named
    ``bucket`` (D-01).
    ...
    origin : datetime or None, optional
        Absolute alignment anchor for the bucket boundaries, bound as a bare
        typed ``%s`` (psycopg infers ``timestamptz``/``timestamp`` from the
        Python ``datetime``). Mutually exclusive with ``offset`` (TSDB-F02).
    offset : str or None, optional
        Relative interval shift for the bucket boundaries (e.g.
        ``"30 minutes"``), bound as ``%s::interval``. Mutually exclusive
        with ``origin`` (TSDB-F02).
    ...
    ------
    ValueError
        If both ``origin`` and ``offset`` are non-``None`` — TimescaleDB has
        no ``time_bucket`` overload accepting both, so this is raised before
        any SQL is built (D-07).
    """
    validate_identifiers(table, schema, time_column)

    if origin is not None and offset is not None:
        raise ValueError(
            "time_bucket: 'origin' and 'offset' are mutually exclusive "
            "(TimescaleDB has no time_bucket overload accepting both); "
            "pass one or neither."
        )

    params: list[Any] = [bucket_width]
    bucket_expr = f"time_bucket(%s, {time_column}"
    if origin is not None:
        bucket_expr += ", origin => %s"
        params.append(origin)
    elif offset is not None:
        bucket_expr += ', "offset" => %s::interval'
        params.append(offset)
    bucket_expr += ")"

    sql = f"SELECT {bucket_expr} AS bucket, {aggregates} FROM {schema}.{table}"
...
class TimescaleAccessor:
    ...
    def compress_chunk(self, chunk: str, *, if_not_compressed: bool = True) -> None:
        """Compress a single named chunk (Community license feature).

        ``chunk`` is the fully-qualified chunk name as returned by
        :meth:`show_chunks` (e.g. ``"_timescaledb_internal._hyper_1_2_chunk"``),
        bound as ``%s::regclass`` (TSDB-F03, RESEARCH D-10/D-11) — it is a
        value, not a SQL identifier, so ``validate_identifiers`` does not
        apply here; injection safety comes from parameter binding alone.

        Parameters
        ----------
        chunk : str
            Fully-qualified chunk name (as returned by :meth:`show_chunks`).
        if_not_compressed : bool, optional
            If ``True`` (default), silently skip if the chunk is already
            compressed instead of raising.
        ...
        self._db.execute(
            "SELECT compress_chunk(%s::regclass, if_not_compressed => %s)",
            [chunk, if_not_compressed],
        )

    def decompress_chunk(self, chunk: str, *, if_compressed: bool = True) -> None:
        """Decompress a single named chunk (Community license feature).

        ``chunk`` is the fully-qualified chunk name as returned by
        :meth:`show_chunks`, bound as ``%s::regclass`` (TSDB-F03, RESEARCH
        D-10/D-11) — it is a value, not a SQL identifier, so
        ``validate_identifiers`` does not apply here; injection safety comes
        from parameter binding alone.

        Parameters
        ----------
        chunk : str
            Fully-qualified chunk name (as returned by :meth:`show_chunks`).
        if_compressed : bool, optional
            If ``True`` (default), silently skip if the chunk is already
            decompressed instead of raising.
        ...
        self._db.execute(
            "SELECT decompress_chunk(%s::regclass, if_compressed => %s)",
            [chunk, if_compressed],
        )
    ...
    def list_hypertables(self) -> list[dict[str, Any]]:
    ...
    def hypertable_info(self, table: str, *, schema: str = "public") -> dict[str, Any]:
    ...
    def add_dimension(
        ...
        *,
        ...
        if partition_type == "hash":
            # number_partitions is guaranteed not None here — validated above
            # (mypy cannot narrow across the earlier construction-time check).
            assert number_partitions is not None
            dim = f"by_hash('{column}', {int(number_partitions)})"
        else:  # range
            # chunk_interval is guaranteed not None here — validated above
            # (mypy cannot narrow across the earlier construction-time check).
            assert chunk_interval is not None
    ...
    def drop_continuous_aggregate(
        ...
        *,
        ...
        Uses a plain ``self._db.execute`` call (TSDB-F01, RESEARCH D-05 —
        ``DROP MATERIALIZED VIEW`` is ordinary transaction-safe DDL, unlike
        :meth:`create_continuous_aggregate` / :meth:`refresh_continuous_aggregate`
        which require the autocommit seam). Dropping a continuous aggregate
        also removes its refresh policies.

        Parameters
        ----------
        view_name : str
            Name of the continuous-aggregate view to drop.
        cascade : bool, optional
            If ``True``, adds ``CASCADE`` to the drop statement, also
            dropping any dependent objects. By default ``False``.
        if_exists : bool, optional
            If ``True``, adds ``IF EXISTS`` so the drop is a no-op when the
            view does not exist. By default ``False`` — a destructive
            operation should be explicit about its target.
        schema : str, optional
            Schema for the view, by default ``"public"``.
        ...
        Notes
        -----
        **DESTRUCTIVE / IRREVERSIBLE.** Dropped continuous aggregates and
        their materialized data cannot be recovered unless you have a
        backup.
        """
        validate_identifiers(view_name, schema)
        ...
        self._db.execute(f"DROP MATERIALIZED VIEW {ie}{schema}.{view_name}{cas}")
    ...
    def remove_continuous_aggregate_policy(
        ...
        *,
        ...
        Symmetric counterpart to :meth:`add_continuous_aggregate_policy`
        (TSDB-F01). Uses a plain ``self._db.execute`` call (D-01 —
        transaction-safe, like ``add_continuous_aggregate_policy``; NOT the
        autocommit seam).

        Parameters
        ----------
        view_name : str
            Name of the continuous-aggregate view to remove the policy from.
        if_exists : bool, optional
            If ``True`` (default), silently skip if no policy exists for
            this view. If ``False``, the DB raises an error when absent.
        schema : str, optional
            Schema for the view, by default ``"public"``.
        ...
        ie = "true" if if_exists else "false"
        self._db.execute(
            f"SELECT remove_continuous_aggregate_policy("
            f"'{schema}.{view_name}', if_exists => {ie})"
        )

    def _run(
        self, sql: str, params: list[Any], into: str
    ...
    def time_bucket(
        ...
        *,
        ...
        schema : str, optional
            Schema name, by default ``"public"``.
        origin : datetime or None, optional
            Absolute alignment anchor for the bucket boundaries, bound as a
            bare typed ``%s``. Mutually exclusive with ``offset`` (TSDB-F02).
        offset : str or None, optional
            Relative interval shift for the bucket boundaries (e.g.
            ``"30 minutes"``), bound as ``%s::interval``. Mutually exclusive
            with ``origin`` (TSDB-F02).
        ...
        ValueError
            ...
            call (D-03). Also raised if both ``origin`` and ``offset`` are
            non-``None`` — mutually exclusive, raised before any DB
            round-trip (D-07, TSDB-F02).
        ExtensionNotAvailableError
            If the TimescaleDB extension is not installed.
        """
        if where is not None:
...
class AsyncTimescaleAccessor:
    ...
    async def compress_chunk(
        self, chunk: str, *, if_not_compressed: bool = True
    ) -> None:
        """Compress a single named chunk (Community license feature).
    ...
    async def decompress_chunk(self, chunk: str, *, if_compressed: bool = True) -> None:
        """Decompress a single named chunk (Community license feature).
    ...
    async def drop_continuous_aggregate(
        self,
        view_name: str,
        *,
        cascade: bool = False,
        ...
        Async mirror of :meth:`TimescaleAccessor.drop_continuous_aggregate`.
        Uses a plain ``await self._db.execute`` call (TSDB-F01, RESEARCH D-05
        — ``DROP MATERIALIZED VIEW`` is ordinary transaction-safe DDL, unlike
        :meth:`create_continuous_aggregate` / :meth:`refresh_continuous_aggregate`
        which require the autocommit seam). Dropping a continuous aggregate
        also removes its refresh policies.
        ...
        **DESTRUCTIVE / IRREVERSIBLE.** Dropped continuous aggregates and
        ...
        ie = "IF EXISTS " if if_exists else ""
        cas = " CASCADE" if cascade else ""
        await self._db.execute(f"DROP MATERIALIZED VIEW {ie}{schema}.{view_name}{cas}")

    async def remove_continuous_aggregate_policy(
        self,
        view_name: str,
        *,
        if_exists: bool = True,
        ...
        Async mirror of
        :meth:`TimescaleAccessor.remove_continuous_aggregate_policy`.
        Symmetric counterpart to :meth:`add_continuous_aggregate_policy`
        (TSDB-F01). Uses a plain ``await self._db.execute`` call (D-01 —
        transaction-safe; NOT the autocommit seam).
